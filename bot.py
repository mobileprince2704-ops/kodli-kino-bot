...
class Database:
    ...
    def add_channel(self, channel: str) -> bool:
        """Yangi majburiy kanal qo'shadi. Agar allaqachon mavjud bo'lsa False qaytaradi."""
        channels = self.get_channels()
        if channel in channels:
            return False
        channels.append(channel)
        self.set_setting("required_channels", json.dumps(channels))
        return True

    def remove_channel(self, index: int) -> Optional[str]:
        """Berilgan index bo'yicha kanalni ro'yxatdan o'chiradi va nomini qaytaradi."""
        channels = self.get_channels()
    ...
    def add_referral(self, new_user_id: int, referrer_id: int) -> bool:
        ...
        if not row or row["referred_by"] or not referrer:
            return False
        # Yangi userga "kim taklif qildi" yoziladi (avval referrer_id ga yozilardi — xato edi).
        self.connection.execute(
            "UPDATE users SET referred_by = ? WHERE user_id = ?",
            (referrer_id, new_user_id),
        )
        # Bonus va hisoblagich esa taklif qiluvchiga beriladi.
        self.connection.execute(
            """
            UPDATE users
...
def channel_menu_keyboard() -> ReplyKeyboardMarkup:
    """Majburiy kanallarni boshqarish menyusi."""
    return ReplyKeyboardMarkup(
        keyboard=[
...
def channel_delete_keyboard(channels: list[str]) -> InlineKeyboardMarkup:
    """Har bir kanal uchun alohida o'chirish tugmasi bilan inline menyu."""
    rows = [
        [InlineKeyboardButton(text=f"❌ {channel}", callback_data=f"delch:{index}")]
...
async def channel_menu_open(message: Message, state: FSMContext) -> None:
    if not is_admin(message.from_user.id):
        return
    await state.clear()
    channels = db.get_channels()
    label = "\n".join(f"• {escape(c)}" for c in channels) if channels else "😔 Hozircha majburiy kanal yo'q."
    await message.answer(
        "📢 <b>Majburiy kanallar</b>\n\n"
        f"{label}\n\n"
        "Quyidagi tugmalar orqali kanal qo'shing yoki o'chiring:",
...
async def channel_add_start(message: Message, state: FSMContext) -> None:
    if not is_admin(message.from_user.id):
        return
    await state.set_state(ChannelAddStates.channel)
    await message.answer(
        "➕ Yangi majburiy kanalni yuboring.\n\n"
        "Namuna: <code>@kanal_username</code> yoki <code>https://t.me/kanal_username</code>\n"
        "⚠️ Bot bu kanalda <b>administrator</b> bo'lishi shart, aks holda obunani tekshira olmaydi.",
        reply_markup=cancel_keyboard(),
    )


@router.message(ChannelAddStates.channel)
async def channel_add_handler(message: Message, state: FSMContext) -> None:
    if await cancel_if_requested(message, state):
        return
    channel = (message.text or "").strip()
    if not (channel.startswith("@") or channel.startswith("https://t.me/")):
        await message.answer("⚠️ Kanal @username yoki https://t.me/ ko'rinishida bo'lishi kerak.")
        return
    added = db.add_channel(channel)
    await state.clear()
    if added:
        await message.answer(
            f"✅ Kanal qo'shildi: {escape(channel)}",
            reply_markup=channel_menu_keyboard(),
        )
    else:
        await message.answer(
            "⚠️ Bu kanal ro'yxatda allaqachon bor.",
            reply_markup=channel_menu_keyboard(),
        )


@router.message(F.text == "🗑 Kanal o'chirish")
async def channel_delete_list(message: Message) -> None:
    if not is_admin(message.from_user.id):
        return
    channels = db.get_channels()
    if not channels:
        await message.answer("😔 O'chirish uchun kanal yo'q.", reply_markup=channel_menu_keyboard())
        return
    await message.answer(
...
async def channel_delete_callback(callback: CallbackQuery) -> None:
    ...
    try:
        ...
        return
    removed = db.remove_channel(index)
    if removed is None:
        await callback.answer("Bu kanal allaqachon o'chirilgan bo'lishi mumkin.", show_alert=True)
        return
    channels = db.get_channels()
    if channels:
        ...
        await callback.message.edit_text("✅ Barcha majburiy kanallar o'chirildi.")
    await callback.answer(f"❌ O'chirildi: {removed}")


@router.message(F.text == "📋 Kanallar ro'yxati")
async def channel_list_handler(message: Message) -> None:
    if not is_admin(message.from_user.id):
        return
    channels = db.get_channels()
    label = (
        "\n".join(f"{i + 1}. {escape(c)}" for i, c in enumerate(channels))
        if channels
        else "😔 Hozircha majburiy kanal yo'q."
    )
    await message.answer(f"📋 <b>Majburiy kanallar</b>\n\n{label}", reply_markup=channel_menu_keyboard())


@router.message(F.text == "⬅️ Admin panel")
async def channel_menu_back(message: Message, state: FSMContext) -> None:
    if not is_admin(message.from_user.id):
        return
    await state.clear()
    await message.answer("👑 <b>Admin panel</b>\nKerakli amalni tanlang:", reply_markup=admin_keyboard())
